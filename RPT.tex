...
시스템의 데이터 버스의 크기는 8비트 이며 주소 버스의 크기는 RAM의 용량에 따라 변한다. 본 연구에서는 256KB 크기의 62256 SRAM을 이용하였으며 주소버스의 크기가 14비트이기 때문에 데이터버스 8핀 + 주소버스 14핀 + 하드웨어 제어에 필요한 나머지 핀까지 해서 프로세서에는 총 22개이상의 입출력이 가능한 핀이 있어야 한다. 
...
두개의 MCU가 공유 메모리에 접근해서 충돌없이 프로세서가 각자 작업을 수행하기 위해서는 RAM 내부의 영역을 프로세서1과 프로세서2의 영역 두개로 나누어야 한다고 판단하였다. 
