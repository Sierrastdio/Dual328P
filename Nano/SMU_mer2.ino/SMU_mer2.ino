/*
 * ============================================================================
 * Arduino Nano #1 - SMU v3.0 (Pin-Map Compliant, PORT Control)
 * ============================================================================
 * 핀 배치:
...
uint16_t calcPhysicalAddr(uint8_t page, uint8_t offset) {
    uint16_t addr = ((uint16_t)page << 8) | offset;
...
void handleCommand() {
    ...
    if (cmd.startsWith(":w ")) {
        ...
        for(uint16_t i = 0; i < prog_sz; i++) {
            uint8_t page_offset = i % 256;
            uint8_t write_page = current_page + (i / 256);
