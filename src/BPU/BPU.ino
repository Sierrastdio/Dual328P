 ...
 * - :boot  releases cores for 1s to receive EEPROM timing, then resets
...
void setup() {
    ...
    RESET_CORES();          // ★ 항상 리셋 상태로 시작 — 부팅 윈도우 없음 ★
    Serial.println(F("BPU Mega v3.0 Ready"));
    Serial.println(F("Commands: :boot :run :rst :wb <bank> <page> <total> :timing :timing1 :timing2"));
}
...
void loop() {
    ...
    // ── :boot ─────────────────────────────────────────────────────────────
    // 코어를 1초 해제 → EEPROM 타이밍 출력 수신 → 다시 리셋
    if (cmd == ":boot") {
        core1_timing[0] = '\0';
        core2_timing[0] = '\0';
        RELEASE_CORES();
        Serial.println(F("Boot window open (1000ms)..."));
        ...
        RESET_CORES();
        Serial.println(F("Boot window closed. Send :run to start."));
        return;
    }
