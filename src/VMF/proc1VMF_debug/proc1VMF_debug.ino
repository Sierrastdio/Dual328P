...
#define EEPROM_ADDR_STK_OVF     ((uint8_t*) 0x0A)
#define EEPROM_ADDR_STK_UDF     ((uint8_t*) 0x0B)
#define EEPROM_ADDR_C2_TIMEOUT  ((uint16_t*)0x0C)
...
static uint8_t  dbg_stack_underflow = 0;
...
void timing_save_eeprom() {
    ...
    eeprom_update_byte (EEPROM_ADDR_STK_OVF,   dbg_stack_overflow);
    eeprom_update_byte (EEPROM_ADDR_STK_UDF,   dbg_stack_underflow);
    eeprom_update_word (EEPROM_ADDR_C2_TIMEOUT, dbg_core2_timeout);
    eeprom_update_dword(EEPROM_ADDR_HANDOFF,    dbg_bus_handoff);
...
void timing_load_and_print_eeprom() {
    ...
    uint8_t  s_ovf   = eeprom_read_byte (EEPROM_ADDR_STK_OVF);
